...
class SteeringBlock(nn.Module):
    ...
    def forward(
        ...
        if self.expert_block is not None:
            expert_mean = torch.mean(expert_outs[0], dim=2, keepdim=True)
            expert_var = torch.var(expert_outs[0], dim=2, keepdim=True)

        if self.anti_expert_block is not None:
            antiexpert_mean = torch.mean(anti_expert_outs[0], dim=2, keepdim=True)
        ...
        if self.anti_expert_block is not None:
            anti_expert_outs_norm = (anti_expert_outs[0] - antiexpert_mean) / torch.sqrt(antiexpert_var)
        else:
        ...
        ensemble_outs = self.alpha_base * base_outs
